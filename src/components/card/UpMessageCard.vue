        ...
        <div class="flex justify-start items-start gap-2 flex-grow">
          ...
          <div class="">
            <div class="font-bold line1">
              {{ message.nickname }} <span class="font-[500]">等 {{ message.num }} 人</span>
            </div>
            <div>赞了你的{{ type(message.target_type.toNumber()) }}</div>
          </div>
        </div>

        <div class="line2 w-16">{{ message.content }}</div>
      </div>
    </div>
  </var-list>
</template>
...
const messages = ref<Types.UpMessageInfoStructOutput[]>([])
const loading = ref(false)
const finished = ref(false)
const contract = assertNotEmpty(store.contract, "合约未初始化")

const load = async () => {
  const res = await contract.getUpMessages(via.MESSAGE, tail_address)
  for (const up_message_info of res.up_message_infos) {
    if (up_message_info.target_address === zero_address) break
    messages.value.push(up_message_info)
